...
def resolve_unmatched_movement_finding(finding_id: int, resolution_note: str) -> dict:
    """Resolve an unmatched_movement finding without touching any payment table.
    ...
    if finding["status"] != "open":
        raise ValueError("not_open")
    if finding["finding_type"] != "unmatched_movement":
        raise ValueError("not_unmatched_movement")
    if not resolution_note or not resolution_note.strip():
        raise ValueError("note_required")
